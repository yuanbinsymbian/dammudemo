...
def main():
    if len(sys.argv) < 5:
        print("usage: test_group_query.py <domain> <app_id> <room_id> <open_id> [secret]")
        sys.exit(1)
    domain, app_id, room_id, open_id = sys.argv[1:5]
    secret = sys.argv[5] if len(sys.argv) >= 6 else ""
